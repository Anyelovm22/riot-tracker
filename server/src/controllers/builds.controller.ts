...
const INSIGHTS_CACHE_TTL_MS = 1000 * 60 * 3;
const insightsCache = new Map<string, { expiresAt: number; payload: any }>();

function getInsightsCacheKey(params: {
  champion: string;
  platform: string;
...
export async function getChampionBuildInsights(req: Request, res: Response) {
  try {
    ...
    const maxPlayers = Math.min(15, Math.max(3, Number(req.query.maxPlayers || 8)));
    const maxMatchesPerPlayer = Math.min(15, Math.max(3, Number(req.query.maxMatchesPerPlayer || 10)));
    ...
    const matchesByPlayer = await Promise.all(
      topPlayers.map(async (entry: any) => {
        const matchIds = await getMatchIdsByPuuid(entry.puuid, platform, maxMatchesPerPlayer, 0).catch(() => []);
        const matches = await Promise.all(
          matchIds.map(async (id: string) => ({
            id,
            match: await getMatchById(id, platform).catch(() => null),
          }))
        );
    ...
    for (const playerBundle of matchesByPlayer) {
      const { entry, matches } = playerBundle;
