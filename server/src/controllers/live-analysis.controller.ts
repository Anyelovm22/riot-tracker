...
function buildBuildAdvice(
  ...
  const decayCandidates = getDecayCandidates(me, enemy, comparison);
  ...
  if (shouldSwapBoots(me.items || [])) {
    ...
    if (!alreadyOwnItem(me.items || [], bootSwapTarget)) {
      replaceSuggestions.push({
        sell: 'Botas',
        buy: bootSwapTarget.name,
        buyItemId: bootSwapTarget.id,
        reason:
  ...
  try {
    ...
    const targetForBuildAdvice =
      mode === 'team'
        ...
        : primaryTarget;
    const initialBuildAdvice = buildBuildAdvice(me, targetForBuildAdvice, comparisons[0] || null);

    const buildAdvice: BuildAdvice = {
      sellCandidates: initialBuildAdvice.sellCandidates,
      replaceSuggestions: initialBuildAdvice.replaceSuggestions,
      decayCandidates: initialBuildAdvice.decayCandidates,
      fullBuildTips: [
