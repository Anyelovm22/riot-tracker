...
export default function ChampionBuildsPage() {
  ...
  async function loadInsights(nextVersus?: string, nextRole?: string) {
    try {
      setLoading(true);
      setError('');
      const result = await fetchChampionBuildInsights({
        ...
        role: nextRole && nextRole !== 'ALL' ? nextRole : undefined,
      });
      setData(result);
    } catch (err: any) {
      ...
      <div className="page-container space-y-6">
        ...
        <section className="rounded-3xl border border-[var(--border-default)] bg-[linear-gradient(110deg,rgba(16,185,129,0.14),rgba(9,9,11,0.92)_35%,rgba(59,130,246,0.2))] p-6">
          ...
          <div className="mt-4 flex flex-wrap items-end gap-3">
            <div>
              <label className="text-xs text-[var(--text-muted)]">Línea</label>
              <select
                value={role}
              ...
              >
                ...
                {['TOP', 'JUNGLE', 'MIDDLE', 'BOTTOM', 'UTILITY'].map((line) => (
            ...
            <button
              onClick={() => loadInsights(versusChampion, role)}
          ...
          <>
            ...
            {data.defaultBuild ? (
              <section className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
                <h2 className="text-lg font-semibold text-emerald-200">Build base recomendada ({data.defaultBuild.role})</h2>
                <p className="mt-1 text-sm text-emerald-100/90">
                  Score {data.defaultBuild.score} · WR {data.defaultBuild.winRate}% · KDA prom {data.defaultBuild.avgKda} · CS/min {data.defaultBuild.avgCsPerMin}
                </p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {(data.defaultBuild.items || []).map((item: any) => (
                    <img
                      key={`default-${item.itemId}`}
            ...
            {!!Object.keys(data.byRole || {}).length && (
              <section className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-card)] p-5">
                <h2 className="text-lg font-semibold text-[var(--text-primary)]">Mejores builds por línea</h2>
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  {Object.entries(data.byRole || {}).map(([line, builds]: [string, any]) => (
                    <div key={line} className="rounded-xl bg-[var(--bg-elevated)] p-4">
                      <p className="font-semibold text-[var(--text-primary)]">{line}</p>
                      <div className="mt-3 space-y-3">
                        {(builds || []).map((build: any, index: number) => (
                          <div key={`${line}-${index}`} className="rounded-lg bg-[var(--bg-card)] p-3">
                            <p className="text-xs text-[var(--text-secondary)]">{build.games} partidas · WR {build.winRate}%</p>
                            <div className="mt-2 flex flex-wrap gap-2">
                              {(build.items || []).map((item: any) => (
                                <img
                                  key={`${line}-${index}-${item.itemId}`}
                                  src={getItemIconUrl(ddragonVersion, item.itemId)}
